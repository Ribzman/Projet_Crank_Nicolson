...
psi = np.exp(-0.5 * (X**2 + Y**2) / (2*sigma**2)) * np.exp(1j*theta)*np.exp(1j*theta2)
...
diagA = (1 + 2*rx + 2*ry + 0.5j*dt*V0) * np.ones(N)
offA  = -rx * mask
farA  = -ry * np.ones(N-Ny)
A = diags([farA, offA, diagA, offA, farA],
          [Nx, 1, 0, -1, -Nx], format='csc')

diagB = (1 - 2*rx - 2*ry - 0.5j*dt*V0) * np.ones(N)
offB  =  rx * mask
farB  =  ry * np.ones(N-Ny)
B = diags([farB, offB, diagB, offB, farB],
          [Nx, 1, 0, -1, -Nx], format='csc')
...
def calculate_energy(psi2D):
    ...
    E_kin = 0.5   * np.sum(-np.real(np.conj(p) * laplacian)) * dx * dy
    E_pot =         np.sum(V * density)                       * dx * dy
    E_int = 0.5*g * np.sum(density**2)                        * dx * dy

    return np.real(E_kin + E_pot + E_int)
...
Energy, = ax4.plot([], [], lw=1.2, color='C1')
ax3.set_title('Norme ‖ψ‖²')
ax4.set_title('Énergie E(t)')

fig.suptitle("Vortex precessions in 2D — GPE réelle")
phys_time = 0
step = 1
E= 0
...
def animate(i):
    global psi2D, phys_time, E

    for _ in range(step):
        psi_guess = psi2D.copy()
        for __ in range(3):
            avg_density = 0.5 * (np.abs(psi2D)**2 + np.abs(psi_guess)**2)
            O = diags([0.5j * dt * g * avg_density], [0], format='csc')
            psi_guess = spsolve((A + O), (B - O) @ psi2D)
